...
VELOCIDAD_AVANCE_RAPIDA = 1.2
VELOCIDAD_AVANCE_MEDIA = 0.8
VELOCIDAD_AVANCE_LENTA = 0.4
VELOCIDAD_GIRO_MAXIMA = 0.45
GANANCIA_GIRO = 0.006
FACTOR_SUAVIZADO_ERROR = 0.25
VENTANA_RESULTADO = "Practica 15.2 - Seguimiento de pelota"
...
def calcular_velocidad_avance(error_x):
    ...
    if error_absoluto < 15:
        return VELOCIDAD_AVANCE_RAPIDA
    if error_absoluto < 60:
...
def dibujar_seguimiento(
    ...
    if centro_pelota is not None:
        ...
        texto = "Pelota no detectada - robot parado"
...
try:
    while True:
        ...
        if not captura_guardada:
            cv2.imwrite("mascara_practica15_2.png", mascara)
            cv2.imwrite("captura_practica15_2.png", resultado)
